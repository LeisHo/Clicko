...
function buildElementSelect() {
  ...
  for (const name of groupNames) {
    ...
    if (selectedGroup === name) opt.selected = true;
  ...
  for (const el of ungrouped) {
    ...
    opt.value = 'el:' + el.id;
    opt.textContent = el.id + ' (' + el.role + ')';
    if (!selectedGroup && el.id === selectedElementId) opt.selected = true;
    objectSelect.appendChild(opt);
  }
  objectSelect.addEventListener('change', () => {
    ...
    if (!raw) {
      ...
      // than leaving the Inspector on a blank "select a property" state —
      // one click still gets you to real, editable content.
      selectedGroup = raw.slice('group:'.length);
      const members = groupMembers.get(selectedGroup) || [];
      selectedElementId = members.length ? members[0].id : null;
    } else {
      selectedGroup = null;
      selectedElementId = raw.slice('el:'.length);
    }
    render();
  });
  ...
  if (selectedGroup) {
    ...
    propSelect.addEventListener('change', () => {
      selectedElementId = propSelect.value || null;
      activeContext = 'base';
      addOverridePickerOpen = false;
      render();
    });
