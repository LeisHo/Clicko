...
export function buildElementCanonicalTree(elementId, layoutConfig) {
  ...
  // position
  const mode = layoutConfig.position && layoutConfig.position.mode;
  ...
  ['width', 'height'].forEach((axis, axisIndex) => {
    ...
    const axisMode = layoutConfig.size && layoutConfig.size[axis] && layoutConfig.size[axis].mode;
  ...
  // constraints — always both fields present (constraints are always-optional, not mode-dispatched).
  // No id collision risk with position/size field keys, so this uses the
  // exact same id === fieldKey convention as every other setting node.
